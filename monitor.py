...
class AnkerSolixApiMonitor:
    ...
    def print_device_mqtt(
        ...
        **kwargs,
        ...
        CONSOLE.info(
            f"Active MQTT topics: {Color.GREEN}{str(self.api.mqttsession.subscriptions or ' None ')[1:-1]}{Color.OFF}"
        )
        CONSOLE.info(
    ...
    def print_api_data(self) -> None:  # noqa: C901
        ...
        if vehicles := self.api.account.get("vehicles") or {}:
            CONSOLE.info("=" * 80)
            CONSOLE.info(
                f"{Color.BLUE}Electric vehicle details for user '{self.api.account.get('nickname') or 'Unknown'}':{Color.OFF}"
            )
