    ...
    def customize_cache(self) -> bool:
        ...
        if isinstance(select, int):
            ...
            if value is not None:
                ...
                CONSOLE.info(f"{Color.RED}Customization cancelled!{Color.OFF}")
            ...
            if devtype == SolixDeviceType.COMBINER_BOX.value:
                ...
                for i in range(1, 7):
                    if m1 := cm and mqtt.get(f"device_{i}_sn", ""):
                        ...
                        m4 = cm and str(mqtt.get(f"device_{i}_exp_packs", ""))
                        m5 = cm and mqtt.get(f"device_{i}_type", "")
                        m6 = cm and mqtt.get(f"device_{i}_temperature", "")
                        if m6 and mqtt.get("temp_unit_fahrenheit"):
                            m6 = f"{float(m6) * 9 / 5 + 32:>4} °F"
                        else:
                            m6 = f"{m6 or '---':>4} {'°F' if mqtt.get('temp_unit_fahrenheit') else '°C'}"
                        CONSOLE.info(
                            f"{'Dev ' + str(i) + ('   [' + (c or cm) + m3 + co + ']' if m3 else ''):<{col1}}: "
                            f"{m1 and (c or cm)}{m1 + (' (' + m5 + ')' if m5 else ''):<{col2}}{co} "
                            f"{'Dev ' + str(i) + ' SoC/Exp/T':<{col3}}: {m2 and (c or cm)}{soc}{co} ({m4 and (c or cm)}{m4 or '-'}{co}) /{m6 and (c or cm)}{m6}{co}"
                        )
                        m1 = cm and mqtt.get(f"device_{i}_ac_output_power_signed", "")
                        m2 = cm and mqtt.get(f"device_{i}_battery_power", "")
                        m4 = cm and str(mqtt.get(f"device_{i}_battery_status", ""))
                        if m1 or m2:
                            CONSOLE.info(
                                f"{'Dev ' + str(i) + ' Out Pwr':<{col1}}:{m1 and (c or cm)}{m1 or '----':>5} {unit:<{col2 - 5}}{co} "
                                f"{'Dev ' + str(i) + ' Bat Pwr':<{col3}}:{m2 and (c or cm)}{m2 or '----':>5} {unit}{co} / "
                                f"{m4 and (c or cm)}{get_enum_name(SolixBatteryStatus, m4, 'unknown' if m4 else '-----').title() + ' (' + (m4 or '-') + ')'}{co}"
                            )
                        m1 = cm and mqtt.get(f"device_{i}_pv_power", "")
                        m2 = cm and mqtt.get(f"device_{i}_pv_1_power", "")
                        m4 = cm and mqtt.get(f"device_{i}_pv_2_power", "")
                        if m1 or m2 or m4:
                            CONSOLE.info(
                                f"{'Dev ' + str(i) + ' PV Total':<{col1}}:{m1 and (c or cm)}{m1 or '----':>5} {unit:<{col2 - 5}}{co} "
                                f"{'Dev ' + str(i) + ' PV 1 / 2':<{col3}}:{m2 and (c or cm)}{m2 or '----':>5} {unit}{co} /"
                                f"{m4 and (c or cm)}{m4 or '----':>5} {unit}{co}"
                            )
                ...
                for i in range(1, 13, 2):
                    ...
                    if m1 or m2:
                        ...
                        CONSOLE.info(
                            f"{'Pwr Circ. ' + '-'.join(map(str, [i, *m5])):<{col1}}: {m1 and (c or cm)}{m1 or '----':>4} {unit} (Prio/ID: {(m3 or '--') + '/' + (m7 or '--') + ')':<{col2 - 17}}{co} "
                            f"{'Pwr Circ. ' + '-'.join(map(str, [i + 1, *m6])):<{col3}}: {m2 and (c or cm)}{m2 or '----':>4} {unit} (Prio/ID: {(m4 or '--') + '/' + (m8 or '--')}){co}"
                        )
                    else:
                        break
                ...
                if (
                    ...
                    CONSOLE.info(
                        f"{'Battery SoC/SoH':<{col1}}: {str(m1) and (c or cm)}{soc} /{m3 and (c or cm)}{m3 or ' --.-':>6} {'%':<{col2 - 15}}{co} "
                ...
                energy = f"{str(dev.get('battery_energy', '')) or '----':>5} Wh"
                ...
                for i in range(1, 6):
                    m1 = cm and (
                        mqtt.get(f"exp_{i}_sn", "")
                        or mqtt.get(f"exp_{i}_controller_sn", "")
                    )
                    ...
                    if m1 or m2:
                        ...
                        m1 = cm and str(mqtt.get(f"exp_{i}_battery_status", ""))
                        if m2 := cm and mqtt.get(f"exp_{i}_soh", ""):
                            m2 = f"{float(m2):6.2f}"
                        m4 = dev.get(f"exp_{i}_size", "")
                        if m1 or m2 or m3:
                            CONSOLE.info(
                                f"{'Exp. ' + str(i) + ' Status':<{col1}}: {m1 and (c or cm)}{get_enum_name(SolixBatteryStatus, m1, 'unknown' if m1 else '-----').title() + ' (' + (m1 or '-') + ')':<{col2}}{co} "
                                f"{'Exp. ' + str(i) + ' SoH/Size':<{col3}}: {m2 and (c or cm)}{m2 or '---':>4} %{co} / "
                                f"{cc}{m4 or dev.get('battery_size', '-----')!s:>5} Wh{co}"
                            )
                    else:
                        break
                ...
                m1 = (c or cm) and mqtt.get("photovoltaic_power", "")
                m2 = (c or cm) and mqtt.get("output_power", "")
                m3 = cm and mqtt.get("pv_yield", "")
                m4 = cm and mqtt.get("output_energy", "")
                CONSOLE.info(
                    f"{'Solar Power':<{col1}}: {m1 and (c or cm)}{m1 or dev.get('input_power') or '---':>4} {unit}{m3 and (c or cm)}{((' (' + m3 + ' kWh)') if m3 else ''):<{col2 - 6}}{co} "
                    f"{'Output Power':<{col3}}: {m2 and (c or cm)}{m2 or dev.get('output_power') or dev.get('to_home_load') or '---'!s:>4} {unit}{m4 and (c or cm)}{((' (' + m4 + ' kWh)') if m4 else '')}{co}"
                )
                ...
                m2 = (c or cm) and mqtt.get("heating_power", "")
                ...
                m1 = (c or cm) and mqtt.get("bat_charge_power", "")
                m2 = (c or cm) and mqtt.get("bat_discharge_power", "")
                ...
                m1 = (c or cm) and mqtt.get("battery_power_signed", "")
                m2 = (c or cm) and mqtt.get("home_load_preset", "")
                CONSOLE.info(
                    f"{'Battery Power':<{col1}}: {m1 and (c or cm)}{m1 or dev.get('charging_power') or '---':>4} {unit:<{col2 - 5}}{co} "
                    f"{'Device Preset':<{col3}}: {m2 and (c or cm)}{m2 or preset:>4} {unit}{co}"
                )
                demand = site.get("home_load_power") or ""
                load = (site.get("solarbank_info") or {}).get("to_home_load") or ""
                diff = ""
                if m1 := (c or cm) and str(mqtt.get("home_demand", "")):
                    demand = m1
                if m2 := (c or cm) and str(
                ...
                if demand or load:
                    with contextlib.suppress(ValueError):
                        if float(demand) > float(load):
                            ...
                            diff = "(+)"
                    CONSOLE.info(
                        f"{'Home Demand':<{col1}}: {str(m1) and (c or cm)}{demand or '---':>4} {unit:<{col2 - 5}}{co} "
                        f"{'SB Home Load':<{col3}}: {str(m2) and (c or cm)}{load or '---':>4} {unit}{co}  {diff}"
                    )
                ...
                CONSOLE.info(
                    f"{'Grid Import Pwr':<{col1}}: {m1 and (c or cm)}{m1 or dev.get('grid_to_home_power') or '----':>5} {unit}{m3 and (c or cm)}{(' (' + m3 + ' kWh)' if m3 else ''):<{col2 - 7}}{co} "
                    f"{'Grid Export Pwr':<{col3}}: {m2 and (c or cm)}{m2 or dev.get('photovoltaic_to_grid_power') or '----':>5} {unit}{m4 and (c or cm)}{(' (' + m4 + ' kWh)' if m4 else '')}{co}"
                )
                for i in range(1, 4):
                    m1 = cm and mqtt.get(f"grid_power_signed_l{i}", "")
                    if (m2 := cm and mqtt.get(f"voltage_l{i}", "")) and "." in m2:
                        m2 = f"{float(m2):>6.2f}"
                    if (m4 := cm and mqtt.get(f"current_l{i}", "")) and "." in m4:
                        m4 = f"{float(m4):>6.3f}"
                    if m1 or m2 or m4:
                        CONSOLE.info(
                            f"{'Grid Power L' + str(i):<{col1}}: {m1 and (c or cm)}{m1 or '-----':>5} {unit:<{col2 - 6}}{co} "
                            f"{'Volt/Current L' + str(i):<{col3}}: {m2 and (c or cm)}{m2 or '---.--':>7} V /{m4 and (c or cm)}{(m4 or ' -.---') + ' A'}{co}"
                        )
                ...
                m1 = cm and mqtt.get("grid_power_signed", "")
                if str(m2 := cm and mqtt.get("energy_local_timestamp", "")):
                    m2 = convert_isotimestamp(m2)
                if m1 or m2:
                    CONSOLE.info(
                        f"{'Grid Power':<{col1}}: {m1 and (c or cm)}{m1 or '-----':>5} {unit:<{col2 - 6}}{co} "
                        f"{'Energy Time':<{col3}}: {m2 and (c or cm)}{m2 or '----.--.-- --:--:--'}{co} "
                    )
                for i in range(1, 4):
                    m1 = cm and mqtt.get(f"grid_import_energy_l{i}", "")
                    m2 = cm and mqtt.get(f"grid_export_energy_l{i}", "")
                    if m1 or m2:
                        CONSOLE.info(
                            f"{'Grid Import L' + str(i):<{col1}}: {m1 and (c or cm)}{m1 or '--.---':>9} {'kWh':<{col2 - 10}}{co} "
                            f"{'Grid Export L' + str(i):<{col3}}: {m2 and (c or cm)}{m2 or '--.---':>9} kWh{co}"
                        )
                ...
                if m1 or m2:
                    CONSOLE.info(
                        f"{'Sys Out Pwr L1':<{col1}}: {m1 and (c or cm)}{m1 or '-----':>5} {unit:<{col2 - 6}}{co} "
                ...
                if m1 or m2:
                    CONSOLE.info(
                        f"{'Sys Out Pwr L2':<{col1}}: {m1 and (c or cm)}{m1 or '-----':>5} {unit:<{col2 - 6}}{co} "
                ...
                if m1 or m2:
                    CONSOLE.info(
                        f"{'Sys Out Pwr L3':<{col1}}: {m1 and (c or cm)}{m1 or '-----':>5} {unit:<{col2 - 6}}{co} "
                ...
                if disaster := details.get("disaster_support"):
                    ...
                    if disaster := status.get("current_disaster_detail"):
                        ...
                        if str(
                            m2 := (cm and mqtt.get("backup_charge_time", ""))
                            or disaster.get("charging_time", "")
                        ).isdigit():
                ...
                if "battery_capacity" in dev:
                    CONSOLE.info(
                        f"{'Battery Energy':<{col1}}: {cc}{str(dev.get('battery_energy', '')) or '-----':>5} {'Wh':<{col2 - 6}}{co} "
                        f"{'Capacity':<{col3}}: {cc}{customized.get('battery_capacity') or dev.get('battery_capacity', '-----')!s:>5} Wh{co}"
                    )
                m1 = cm and mqtt.get("battery_soc", "")
                ...
                if m1 or str(m2):
                    ...
                    CONSOLE.info(
                        f"{'Usage Mode':<{col1}}: {m1 and (c or cm)}{get_enum_name(SolixPpsUsageMode, m1, '----').replace('_', ' ').title() + ' (' + (m1 or '-') + ')':<{col2}}{co} "
                        f"{'Storm Guard':<{col3}}: {str(m2) and (c or cm)}{get_enum_name(SolixSwitchMode, m2, '---').upper()} ({str(m2) or '-'}) {m4 or ''!s}{co}"
                    )
                m1 = cm and str(mqtt.get("working_status", ""))
                if m1:
                    CONSOLE.info(
                        f"{'Work Status':<{col1}}: {m1 and (c or cm)}{get_enum_name(SolixPpsWorkStatus, m1, '----').replace('_', ' ').title() + ' (' + (m1 or '-') + ')':<{col2}}{co} "
                    )
                m1 = (
                    cm and str(mqtt.get("usage_mode", ""))
                ...
                if str(m1) or str(m2) or m3 or m4:
                    ...
                    CONSOLE.info(
                        ...
                        f"{m3 and (c or cm)}{get_enum_name(SolixPpsOutputModeV2 if v2 else SolixPpsOutputMode, m3, 'unknown' if m3 else '-----').capitalize().split('_', maxsplit=1)[0] + ' (' + (m3 or '-') + ')':<{col2 - 6}}{co} "
                        f"{'DC Out Ctrl':<{col3}}: {str(m2) and (c or cm)}{get_enum_name(SolixSwitchMode, m2, str(m2) or '---').upper():>3}{co} / "
                        f"{m4 and (c or cm)}{get_enum_name(SolixPpsOutputModeV2 if v2 else SolixPpsOutputMode, m4, 'unknown' if m4 else '-----').capitalize().split('_', maxsplit=1)[0] + ' (' + (m4 or '-') + ')'}{co}"
                    )
                ...
                for i in range(1, 6):
                    if m1 := cm and mqtt.get(f"exp_{i}_soc", ""):
                        ...
                        m4 = dev.get(f"exp_{i}_size", "")
                        CONSOLE.info(
                            f"{'Exp. ' + str(i) + ' SoC/Temp':<{col1}}: {m1 and (c or cm)}{soc}{co} / {m3 and (c or cm)}{m3:<{col2 - 9}}{co} "
                            f"{'Exp. ' + str(i) + ' SoH/Size':<{col3}}: {m2 and (c or cm)}{m2 or '---.-':>5} %{co} / {cc}{m4 or dev.get('battery_size', '-----')!s:>5} Wh{co}"
                        )
                    else:
                        break
                energy = f"{str(dev.get('battery_energy', '')) or '----':>4} Wh"
                if "battery_capacity" in dev:
                ...
                m1 = cm and mqtt.get("ac_output_timer_seconds", "")
                m3 = cm and mqtt.get("ac_output_timer_switch", "")
                if str(m1) or str(m3):
                    m2 = cm and mqtt.get("ac_output_timer_remaining_seconds", "")
                    m4 = cm and mqtt.get("ac_output_timer_remaining_timestamp", "")
                    if str(m1).isdigit():
                    ...
                    CONSOLE.info(
                        f"{'AC Out Timer':<{col1}}: {str(m1) and (c or cm)}{str(m1) or '--:--:--':>8} "
                        f"{str(m3) and (c or cm)}({get_enum_name(SolixSwitchMode, m3, str(m3) or '---').upper() + ')':<{col2 - 10}}{co} "
                        f"{'AC Out Remain':<{col3}}: {m2 and (c or cm)}{str(m2) or '--:--:--':>8}{co}"
                    )
                ...
                if schedule := (c or cm) and mqtt.get("ac_output_schedule"):
                    CONSOLE.info(f"{'-' * 80}")
                    common.print_pps_schedule({"ac_output_schedule": schedule}, c or cm)
